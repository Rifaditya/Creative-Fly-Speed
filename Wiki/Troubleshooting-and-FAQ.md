...
- Server movement checks (`moved-wrongly-threshold`, `max-vehicle-speed`) in `server.properties` may flag players if the server anti-cheat is active. Ensure `server.properties` allows high movement speeds or configure anti-cheat plugins accordingly.
...
- [[Flight Speed Kinematics|26.3-Flight-Speed-Kinematics]]
